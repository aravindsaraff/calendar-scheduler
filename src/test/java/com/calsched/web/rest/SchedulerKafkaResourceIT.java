...
import com.calsched.config.KafkaProperties;
...
class SchedulerKafkaResourceIT {
    ...
    void setup() {
        ...
        SchedulerKafkaResource kafkaResource = new SchedulerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/scheduler-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/scheduler-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
